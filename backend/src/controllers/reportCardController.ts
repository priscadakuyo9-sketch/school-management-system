...
export const generateStudentReportCard = async (req: Request, res: Response) => {
  try {
    ...
    // STUDENT INFO BOX
    const startY = doc.y;
    doc.fontSize(10).font('Helvetica-Bold').text(`NOM & PRÉNOM: ${student.user?.firstName} ${student.user?.lastName}`, 50, startY);
    doc.text(`CLASSE: ${student.class?.name || 'N/A'}`, 50, startY + 15);
    ...
    student.grades.forEach((grade: any) => {
      const coeff = grade.course?.coefficient || 2;
      const points = grade.score * coeff;
      totalPoints += points;
      totalCoeff += coeff;

      doc.fillColor('#000').font('Helvetica').fontSize(9);
      doc.text(grade.course?.name || 'Matière', 50, currentY + 7);
    ...
    res.setHeader('Content-Disposition', `attachment; filename=bulletin_${student.user?.lastName}.pdf`);
    doc.pipe(res);
    doc.end();
